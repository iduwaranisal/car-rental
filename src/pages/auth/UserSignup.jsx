...
const UserSignup = () => {
  ...
  const onSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      signup({ type: 'user', name, email, password })
      navigate(next)
    } catch (err) {
  ...
  return (
    <AuthShell
      ...
      subtitle='Sign up to book cars and manage your reservations.'
      footer={
        <div className='flex items-center justify-between'>
          <span>Already have an account?</span>
          <Link className='text-primary font-medium hover:underline' to={`/login?next=${encodeURIComponent(next)}`}>
            Login
          </Link>
        </div>
      }
